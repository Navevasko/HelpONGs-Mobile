...
export default function CadastroUsuario() {
  ...
  const [Nome, setNome] = useState("aaadassd");
  const [Email, setEmail] = useState("AE@DdasFSA.COM");
  const [Senha, setSenha] = useState("a");
  const [ConfirmSenha, setConfirmSenha] = useState("a");
      ...
      if (Email.includes("@") && Email.includes(".")) {
        if (Senha == ConfirmSenha) {
          ...
          if (postString.includes("200")) {
            ...
            const id = post.data.idUsuario
            console.log(id);
            setIsLoading(false);
            AsyncStorage.setItem("UserLogin", postString)
            navigation.navigate("PerfilUsuario", {idUsuario : id})
          }
        } else {
